                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem', borderBottom: '1px solid var(--border)', paddingBottom: '1rem' }}>
                    ...
                    <div>
                        ...
                        <p className="text-muted" style={{ margin: 0 }}>
                            {isReadOnly ? "Timeline mapping for your synchronized loan vehicle financing." : "Plan your vehicle financing with easy reducing balance logic."}
                        </p>
                    </div>
                </div>
...
const CarLoanCalculator = ({ data, setData, expenseCategories, journeyAdjustments }) => {
    ...
    const activeRaw = expenseCategories?.emi?.carLoan;
    if (activeRaw && typeof activeRaw === 'object' && parseFloat(activeRaw.principal) > 0) {
        loansToRender.push({
            loanKey: 'active',
            title: 'Active Car Loan (Cash Flow synchronized)',
    ...
    const futureLoans = (journeyAdjustments || []).filter(a => a.type === 'loan' && a.loanCategory === 'carLoan');
    ...
    if (loansToRender.length === 0) {
        loansToRender.push({
            loanKey: 'manual',
            title: 'Standalone Car Loan Calculator',
    ...
    return (
        <div>
            {loansToRender.map(loanConfig => (
                <CarLoanEngine key={loanConfig.loanKey} {...loanConfig} />
            ))}
        </div>
    );
};
