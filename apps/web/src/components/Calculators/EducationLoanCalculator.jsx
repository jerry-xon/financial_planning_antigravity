...
import { Calculator, Calendar, DollarSign, TrendingDown, Clock, Info, BookOpen } from 'lucide-react';

const EducationLoanEngine = ({
        ...
        <div className="fade-in" style={{ padding: '1rem' }}>
            <div className="card" style={{ maxWidth: '1400px', margin: '0 auto' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem', borderBottom: '1px solid var(--border)', paddingBottom: '1rem' }}>
                    <BookOpen size={32} color={isReadOnly ? "var(--success)" : "var(--primary)"} />
                    <div>
                        <h1 style={{ margin: 0 }}>{title}</h1>
                        <p className="text-muted" style={{ margin: 0 }}>
                            {isReadOnly ? "Timeline mapping for your synchronized education goal financing." : "Plan your education financing with easy reducing balance logic."}
                        </p>
                    </div>
                </div>
...
const EducationLoanCalculator = ({ calculatorKey = "edu_loan" }) => {
    ...
    const activeRaw = expenseCategories?.emi?.educationLoan;
    if (activeRaw && typeof activeRaw === 'object' && parseFloat(activeRaw.principal) > 0) {
        loansToRender.push({
            loanKey: 'active',
            title: 'Active Education Loan (Cash Flow synchronized)',
    ...
    const futureLoans = (journeyAdjustments || []).filter(a => a.type === 'loan' && a.loanCategory === 'Education Loan');
    ...
    // 3. Check for Fulfillment Matrix Loan Proposals
    loanProposals.forEach((lp, i) => {
        // Assume all goals that spawn loans in Fulfillment without explicit category are implicitly Education Loans, 
        // or we check if the goal name incorporates Education
        if (lp.name?.toLowerCase().includes('education')) {
            loansToRender.push({
                loanKey: `fulfillment_${lp.id}`,
                title: `Fulfillment Proposal: ${lp.name}`,
                loanAmount: parseFloat(lp.principal) || 0,
                interestRate: parseFloat(lp.rate) || 0,
                tenureYears: (parseFloat(lp.tenure) || 0) / 12,
                startMonth: parseInt(lp.startMonth) || 1,
                startYear: parseInt(lp.startYear) || new Date().getFullYear(),
                isReadOnly: true
            });
        }
    });

    // 4. Standalone Manual Calculator
    if (loansToRender.length === 0) {
        loansToRender.push({
            loanKey: 'manual',
            title: 'Standalone Education Loan Calculator',
            loanAmount: data?.amount ?? 0,
            interestRate: data?.rate ?? 9.5,
    ...
    return (
        <div>
            {loansToRender.map(loanConfig => (
                <EducationLoanEngine key={loanConfig.loanKey} {...loanConfig} />
            ))}
        </div>
    );
};
